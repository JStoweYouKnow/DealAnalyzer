...
export class BlobStorageService {
  ...
  static async getStats() {
    try {
      ...
      const totalSize = files.reduce((sum: number, file: { size: number }) => sum + file.size, 0);
      const sizeInMB = (totalSize / 1024 / 1024).toFixed(2);

      return {
        totalFiles: files.length,
        totalSize: `${sizeInMB} MB`,
        files: files.map((f: { pathname: string; size: number; uploadedAt?: Date }) => ({
          pathname: f.pathname,
          size: `${(f.size / 1024).toFixed(2)} KB`,
          uploadedAt: f.uploadedAt ? new Date(f.uploadedAt).toLocaleString() : undefined,
        })),
      };
    } catch (error) {
